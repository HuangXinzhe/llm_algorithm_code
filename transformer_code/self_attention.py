...
def MultiheadAttentionBatched():
    d_model, seq_len_kv, seq_len, b, h, d_k, d_v = 512, 10, 10, 32, 8, (
        512 // 8), (512 // 8)
...
def MultiquerySelfAttentionIncremental():
    ...
    q = torch.einsum("bd,hdk->bhk", X, P_q)
    K = torch.concat([prev_K, torch.einsum(
        "bd,dk->bk", M, P_k).unsqueeze(1)], axis=1)
    V = torch.concat([prev_V, torch.einsum(
        "bd,dv->bv", M, P_v).unsqueeze(1)], axis=1)
